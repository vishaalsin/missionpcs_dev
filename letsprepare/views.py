        ...
        availableQuizzes = json.loads(
            json.dumps(
                AvailableQuizzesSerializer(
                    AvailableQuizzes.objects.filter(user=user, successful=True),
                    many=True,
                ).data
            )
        )
...
def show_all_on_sale(request):
    ...
    for module in list(LearningModule.objects.all()):
            ...
            quiz_data.append(
                {
                    ...
                    "code": quiz.quiz_code,
                    "price": quiz.price * ((100 - discoun) / 100),
                    "org_price": quiz.price,
                    "id": quiz.id,
                }
            )
            if quiz.id in availableQuizIds or quiz.is_free:
        ...
        module_price = sum(
            [quiz["price"] for quiz in quiz_data if not quiz["available"]]
        )
        modules_data.append(
            {
                "name": module.description,
...
def assign_quizzes(request):
    ...
    try:
        ...
        amount = sum(
            [
                ...
                for quiz in Quiz.objects.filter(
                    id__in=[quiz for quiz in results["quizzes"]]
                )
            ]
        )
        ...
        for quiz in results["quizzes"]:
            data = {"user": request.user.id, "quiz": quiz, "order_id": order_id}
        ...
        if order_status == "created":
            ...
            if settings.IS_DEVELOPMENT:
                context[
                    "callback_url"
                ] = "http://127.0.0.1:8000/letsprepare/verify_payment/"
            else:
                context[
                    "callback_url"
                ] = "https://missionpcs.com/letsprepare/verify_payment/"
...
def show_results(request):
    ...
    for qp in set(question_papers_attempted):
        question_papers_data.append(
            {"code": qp.quiz.quiz_code, "name": qp.quiz.description, "id": qp.id}
        )
    ...
    plot_div = plot(
        ...
        output_type="div",
        config=dict(
            displayModeBar=False, dragMode=False, scrollZoom=False, staticPlot=True
        ),
    ...
    for qp, ap in qa_tups_ordered:
        ...
        if qp.quiz.quiz_code in attempt_dict.keys():
            if num_attempted == 0:
                ...
                attempt_dict[qp.quiz.quiz_code].append(
                    round(
                        ((ap.marks_obtained) / len(ap.questions_answered.all()) * 100),
                        2,
                    )
                )
        else:
...
def send_sms(body, number):
    message = tw_client.messages.create(
        body=body, from_=settings.twilio_number, to=number
    )
...
def verify_payment(request):
    if request.method == "POST":
        ...
        try:
            ...
            # return data_dict
            orders = list(
                AvailableQuizzes.objects.filter(order_id=response["razorpay_order_id"])
            )
            user = orders[0].user
