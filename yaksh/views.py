...
def select_exam(request):
    ...
    if request.method == "POST":
        enrolled_courses_ids = list(
            map(lambda x: request.POST.getlist(x.name), courses)
        )
    ...
    return my_render_to_response(request, "portal_pages/select-exams.html", context)
...
def initiate_user(new_user, enrolled_courses_ids):
    ...
    for quiz in free_quizzes:
        data = {"user": new_user.id, "quiz": quiz.id}
...
def quizlist_user(request, enrolled=None, msg=None):
    ...
    if request.method == "POST":
        ...
        remaining_courses = list(
            Course.objects.filter(active=True, is_trial=False, hidden=False)
    ...
    context = {"user": user, "courses": courses_data, "title": title}
...
def add_question(request, question_id=None):
    ...
    for testcase in TestCase.__subclasses__():
        if test_case_type == testcase.__name__.lower():
            formset = inlineformset_factory(
                Question, testcase, extra=1, fields="__all__", form=TestcaseForm
            )
        else:
            formset = inlineformset_factory(
                Question, testcase, extra=0, fields="__all__", form=TestcaseForm
            )
        formsets.append(formset(instance=question, initial=[{"type": test_case_type}]))
    context = {
        "qform": qform,
        "fileform": fileform,
        "question": question,
...
def add_quiz(request, course_id=None, module_id=None, quiz_id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse("yaksh:edit_quiz", args=[course_id, module_id, added_quiz.id])
            )
    else:
...
def add_exercise(request, course_id=None, module_id=None, quiz_id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if created:
                module.learning_unit.add(unit.id)
            messages.success(request, "{0} saved successfully".format(quiz.description))
            return redirect(
                reverse("yaksh:edit_exercise", args=[course_id, module_id, quiz.id])
            )
    else:
...
def prof_manage(request, msg=None):
    ...
    courses = (
        ...
        .filter(Q(creator=user) | Q(teachers=user), is_trial=False)
        .distinct()
        .order_by("-active")
    )
    ...
    return my_render_to_response(request, "yaksh/moderator_dashboard.html", context)
...
def lp_start(request, questionpaper_id=None):
    ...
    try:
        ...
        return "Quiz not found, please contact your " "instructor/administrator."
...
@email_verified
def start(
    request, questionpaper_id=None, attempt_num=None, course_id=None, module_id=None
):
    """Check the user cedentials and if any quiz is available,
    start the exam."""
    user = request.user
    availableQuizzes = json.loads(
        json.dumps(
            AvailableQuizzesSerializer(
                AvailableQuizzes.objects.filter(user=user), many=True
            ).data
        )
    )
    ...
    try:
        ...
        msg = "Quiz not found, please contact your " "instructor/administrator."
        if is_moderator(user):
    ...
    if not quest_paper.has_questions():
        msg = (
            "Quiz does not have Questions, please contact your "
    ...
    if quest_paper.quiz.is_expired() or not quest_paper.quiz.active:
        msg = "{0} is either expired or not active".format(quest_paper.quiz.description)
        if is_moderator(user) and course.is_trial:
            return prof_manage(request, msg=msg)
        return view_module(request, module_id=module_id, course_id=course_id, msg=msg)
...
def show_question(
    ...
    if question in paper.questions_answered.all():
        notification = (
            ...
            else "You have already attempted this question"
        )
...
def skip(
    ...
    return show_question(
        request, next_q, paper, course_id=course_id, module_id=module_id
    )
...
def check(
    ...
    if request.method == "POST":
        ...
        if current_question.type == "mcq":
            ...
            for fname in assignment_filename:
                ...
                assignment_files = AssignmentUpload.objects.filter(
                    ...
                    course_id=course_id,
                    assignmentFile__icontains=fname,
                    user=user,
                    question_paper=questionpaper_id,
                )
        ...
        if (
            current_question in paper.get_questions_answered()
        ...
        json_data = (
            ...
            if current_question.type == "code" or current_question.type == "upload"
            else None
        )
        result = paper.validate_answer(user_answer, current_question, json_data, uid)
...
def _update_paper(request, uid, result):
    ...
    if result.get("success"):
        new_answer.marks = (
            ...
            if current_question.partial_grading
            and current_question.type == "code"
...
def add_course(request, course_id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_course.save()
            messages.success(request, "Saved {0} successfully".format(new_course.name))
...
def self_enroll(request, course_id):
    ...
    messages.success(
        request,
        "Enrolled in {0} by {1}".format(course.name, course.creator.get_full_name()),
    )
    if is_moderator(user):
...
def courses(request):
    ...
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page")
    courses = (
        Course.objects.filter(Q(creator=user) | Q(teachers=user), is_trial=False)
        .order_by("-active")
        .distinct()
    )
    ...
    if status == "select" and tags:
        courses = courses.filter(name__icontains=tags)
    elif status == "active":
        courses = courses.filter(name__icontains=tags, active=True)
    elif status == "closed":
        courses = courses.filter(name__icontains=tags, active=False)
...
@login_required
@email_verified
def enroll_reject_user(request, course_id, was_enrolled=False, was_rejected=False):
    user = request.user
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page")
    course = get_object_or_404(Course, id=course_id)
...
def send_mail(request, course_id, user_id=None):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("send_mail") == "send_mail":
            ...
            message = send_bulk_mail(subject, email_body, recipients, attachments)
...
@login_required
@email_verified
def show_statistics(request, questionpaper_id, attempt_number=None, course_id=None):
    user = request.user
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page")
...
def monitor(request, quiz_id=None, course_id=None):
    ...
    try:
        ...
        q_paper = QuestionPaper.objects.filter(
            quiz__is_trial=False, quiz_id=quiz_id
        ).distinct()
        ...
        papers = AnswerPaper.objects.filter(
            question_paper_id=q_paper.first().id, course_id=course_id
        ).order_by("user__profile__roll_number")
        ...
        for auser in users:
            ...
            latest_attempts.append(
                papers.get(
                    user__in=auser, attempt_number=last_attempt["last_attempt_num"]
                )
            )
...
def _get_questions_from_tags(question_tags, user, active=True, questions=None):
    ...
    if questions:
        ...
        search = (
            ...
            .filter(tags__name__in=search_tags, user=user, active=active)
            .distinct()
        )
    return search
...
def design_questionpaper(request, course_id, quiz_id, questionpaper_id=None):
    user = request.user
    que_tags = (
        Question.objects.filter(active=True, user=user)
        ...
        if request.POST.get("upload") == "upload":
            ...
            if form.is_valid():
                try:
                    ...
                    unique_rows_of_questions_excel_upload = (
                        modules_with_questions_file.drop_duplicates()
                    )
                    module_and_quiz_wise_grouped_questions = (
                        unique_rows_of_questions_excel_upload.groupby(
                            [
                                "module_name",
                                "module_description",
                                "quiz_description",
                    ...
                    for i, j in module_and_quiz_wise_grouped_questions.iterrows():
                        ...
                        module, module_get_or_create = get_or_create_module(
                            module_to_create, description_module_to_create, user
                        )
                        quiz, quiz_get_or_create = get_or_create_quiz(
                            quiz_description,
                            quiz_code,
                            quiz_duration,
                            price,
                        ...
                        quiz_unit, quiz_unit_get_or_create = get_or_create_quiz_unit(
                            quiz
                        )
...
def get_or_create_module(module_to_create, description_module_to_create, user):
    existing_module = LearningModule.objects.filter(
        creator_id=user, name=module_to_create
    )
    if len(existing_module) > 0:
        existing_module_as_list = list(existing_module)
        return existing_module_as_list[0], 0
    else:
        learning_module_to_create = create_learning_module(
            description_module_to_create, module_to_create, user
        )
...
def get_or_create_quiz(
    quiz_description, quiz_code, quiz_duration, price, is_free, user
):
    existing_quiz = Quiz.objects.filter(
        quiz_code=quiz_code, active=True, creator_id=user
    )
    if len(existing_quiz) > 0:
        existing_quiz_as_list = list(existing_quiz)
        return existing_quiz_as_list[0], 0
    else:
        quiz_to_create = create_quiz(
            quiz_description, quiz_code, quiz_duration, price, is_free, user
        )
...
def get_or_create_quiz_unit(quiz_to_create):
    existing_quiz_unit = LearningUnit.objects.filter(
        order=1, type="quiz", quiz=quiz_to_create, check_prerequisite=False
    )
...
def create_and_save_question_paper(existing_quiz, questions_for_quiz, user):
    existing_question_paper = QuestionPaper.objects.filter(
        quiz=existing_quiz, shuffle_questions=False
    )
    existing_questions = Question.objects.filter(
        active=True, summary__in=questions_for_quiz, user=user
    )
...
def search_questions_by_tags(request):
    ...
    questions = _get_questions_from_tags(question_tags, user, questions=questions)
    paginator = Paginator(questions, 30)
    page = request.GET.get("page")
    questions = paginator.get_page(page)
    context = {
...
def download_quiz_csv(request, course_id, quiz_id):
    ...
    response[
        ...
        course.name.replace(".", ""), quiz.description.replace(".", ""), attempt_number
    )
    ...
    for question in questions:
        questions_scores[
            "{0}-{1}".format(question.summary, question.points)
    ...
    if quiz_id is not None:
        ...
        if user_id is not None:
            ...
            has_user_assignments = AssignmentUpload.objects.filter(
                question_paper_id__in=questionpaper_id, user_id=user_id
            ).exists()
            user = User.objects.get(id=user_id)
...
def search_teacher(request, course_id):
    ...
    if request.method == "POST":
        ...
        if not len(u_name) == 0:
            ...
            ).exclude(Q(id=user.id) | Q(is_superuser=1) | Q(id=course.creator.id))
...
def remove_teachers(request, course_id):
    ...
    user = request.user
    course = get_object_or_404(Course, pk=course_id)
    if not is_moderator(user) and (
        not course.is_creator(user) and course.is_teacher(user)
    ):
        raise Http404("You are not allowed to view this page!")
...
def test_mode(user, godmode=False, questions_list=None, quiz_id=None, course_id=None):
    ...
    if questions_list is not None:
        ...
        trial_unit, created = LearningUnit.objects.get_or_create(
            order=1, type="quiz", quiz=trial_quiz, check_prerequisite=False
        )
...
def test_quiz(request, mode, quiz_id, course_id=None):
    ...
    if (quiz.is_expired() or not quiz.active) and not godmode:
        messages.warning(
            request, "{0} is either expired or inactive".format(quiz.description)
        )
        return my_redirect("/exam/manage")
...
def get_pie_charts(answer_papers):
    ...
    for ap in answer_papers:
        ...
        values = [
            correct,
            len(ap.questions_answered.all()) - correct,
            len(ap.questions_unanswered.all()),
        ]
        trace = go.Pie(
            labels=labels, values=values, hole=0.2, marker=dict(colors=colors)
        )
...
def view_answerpaper(request, questionpaper_id, course_id):
    ...
    if quiz.view_answerpaper and user in course.students.all():
        ...
        has_user_assignment = AssignmentUpload.objects.filter(
            user=user, course_id=course.id, question_paper_id=questionpaper_id
        ).exists()
...
@email_verified
def regrade(
    request, course_id, questionpaper_id, question_id=None, answerpaper_id=None
):
    user = request.user
    course = get_object_or_404(Course, pk=course_id)
    if not is_moderator(user) or (course.is_creator(user) and course.is_teacher(user)):
        raise Http404("You are not allowed to view this page!")
    ...
    messages.info(request, msg)
    return redirect(reverse("yaksh:grade_user", args=[quiz.id, course_id]))
...
def download_course_csv(request, course_id):
    ...
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page!")
    course = Course.objects.prefetch_related("learning_module").get(id=course_id)
    if not course.is_creator(user) and not course.is_teacher(user):
        raise Http404("The question paper does not belong to your course")
    students = (
        course.get_only_students()
        .annotate(
            roll_number=F("profile__roll_number"), institute=F("profile__institute")
        )
        .values("id", "first_name", "last_name", "email", "institute", "roll_number")
    )
    ...
    header = (
        ["first_name", "last_name", "roll_number", "email", "institute"]
        + [quiz.description for quiz in quizzes]
        + ["total_scored", "out_of"]
    )
...
def new_activation(request, email=None):
    ...
    try:
        ...
        context["email_err_msg"] = (
            "Multiple entries found for this email " "Please change your email"
        )
        return my_render_to_response(request, "yaksh/activation_status.html", context)
    ...
    if not user.profile.is_email_verified:
        ...
        success, msg = send_user_mail(
            new_user_data.email, new_user_data.profile.activation_key
        )
        if success:
            context["activation_msg"] = msg
        else:
...
def download_assignment_file(
    request, quiz_id, course_id, question_id=None, user_id=None
):
    user = request.user
    course = get_object_or_404(Course, pk=course_id)
    if (
        not course.is_creator(user)
        and not course.is_teacher(user)
        and not course.is_student(user)
    ):
        raise Http404(
            "You are not allowed to download files for {0}".format(course.name)
        )
    ...
    for f_name in assignment_files:
        ...
        folder_name = os.sep.join(
            (folder, sub_folder, os.path.basename(f_name.assignmentFile.name))
        )
    ...
    if request.method == "POST":
        ...
        try:
            reader = csv.DictReader(
                csv_file.read().decode("utf-8").splitlines(), dialect=dialect
            )
        except TypeError:
    ...
    for row in reader:
        ...
        if users.exists():
            ...
            if remove.strip().lower() == "true":
                ...
                messages.info(
                    request,
                    "{0} -- {1} -- User rejected".format(counter, user.username),
                )
            else:
        ...
        messages.info(
            request,
            "{0} -- {1} -- User {2} Successfully".format(counter, user.username, state),
        )
    if counter == 0:
...
def _get_csv_values(row, fields):
    ...
    remove = "false"
    email, first_name, last_name = map(
        str.strip, [row["email"], row["firstname"], row["lastname"]]
    )
    password = email
    username = email
...
def download_sample_csv(request):
    ...
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page!")
    csv_file_path = os.path.join(FIXTURES_DIR_PATH, "sample_user_upload.csv")
    with open(csv_file_path, "rb") as csv_file:
        response = HttpResponse(csv_file.read(), content_type="text/csv")
        response["Content-Disposition"] = 'attachment; filename="sample_user_upload"'
        return response
...
def duplicate_course(request, course_id):
    ...
    if course.is_teacher(user) or course.is_creator(user):
        ...
        msg = dedent(
            ...
            your instructor/administrator.""".format(
                course.name
            )
        )
        messages.warning(request, msg)
...
def download_yaml_template(request):
    ...
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page!")
    template_path = os.path.join(
        os.path.dirname(__file__), "fixtures", "demo_questions.zip"
    )
...
def download_template_modules(request):
    ...
    if not is_moderator(user):
        raise Http404("You are not allowed to view this page!")
    csv_file_path = os.path.join(FIXTURES_DIR_PATH, "create_modules.csv")
    with open(csv_file_path, "rb") as csv_file:
        response = HttpResponse(csv_file.read(), content_type="text/csv")
        response["Content-Disposition"] = 'attachment; filename="sample_modules_upload"'
        return response
    ...
    if request.method == "POST":
        if "Save" in request.POST:
            ...
            if lesson_form.is_valid():
                ...
                if created:
                    module.learning_unit.add(unit.id)
                messages.success(request, "Saved {0} successfully".format(lesson.name))
                return redirect(
                    reverse("yaksh:edit_lesson", args=[course_id, module_id, lesson.id])
                )
            else:
        ...
        if "Delete" in request.POST:
            ...
            if remove_files_id:
                ...
                for file in files:
                    file.remove()
                messages.success(request, "Deleted files successfully")
            else:
                messages.warning(request, "Please select atleast one file to delete")
...
def show_lesson(request, lesson_id, module_id, course_id):
    ...
    context = {
        "lesson": learn_unit.lesson,
        "user": user,
        "course": course,
        "state": "lesson",
        "all_modules": all_modules,
        "learning_units": learning_units,
        "current_unit": learn_unit,
    ...
    if request.method == "POST":
                ...
                for order, value in enumerate(add_values, start_val):
                    ...
                    if type == "quiz":
                        unit, status = LearningUnit.objects.get_or_create(
                            order=order, quiz_id=learning_id, type=type
                        )
                    else:
                        unit, status = LearningUnit.objects.get_or_create(
                            order=order, lesson_id=learning_id, type=type
                        )
        ...
        if "Change" in request.POST:
            ...
            if order_list:
                ...
                messages.warning(request, "Please select a lesson/quiz to change")
        ...
        if "Remove" in request.POST:
            ...
            if remove_values:
                ...
                messages.success(request, "Lessons/quizzes deleted successfully")
            else:
                messages.warning(request, "Please select a lesson/quiz to remove")
        ...
        if "Change_prerequisite" in request.POST:
            ...
            if unit_list:
                for unit in unit_list:
                    ...
                    learning_unit.save()
                messages.success(request, "Changed prerequisite status successfully")
            else:
                messages.warning(
                    request, "Please select a lesson/quiz to change prerequisite"
                )

    added_quiz_lesson = learning_module.get_added_quiz_lesson()
    quizzes = [
        ("quiz", quiz) for quiz in Quiz.objects.filter(creator=user, is_trial=False)
    ]
    lessons = [("lesson", lesson) for lesson in Lesson.objects.filter(creator=user)]
    ...
    if request.method == "POST":
        if "Save" in request.POST:
            ...
            if module_form.is_valid():
                ...
                # x = Quiz.objects.filter

                module.save()
                course.learning_module.add(module.id)
                messages.success(request, "Saved {0} successfully".format(module.name))
            else:
...
@login_required
@email_verified
def get_next_unit(request, course_id, module_id, current_unit_id=None, first_unit=None):
    user = request.user
    course = Course.objects.prefetch_related("learning_module").get(id=course_id)
    if not course.students.filter(id=user.id).exists():
        raise Http404("You are not enrolled for this course!")
    learning_module = course.learning_module.prefetch_related("learning_unit").get(
        id=module_id
    )

    if current_unit_id:
        current_learning_unit = learning_module.learning_unit.get(id=current_unit_id)
    else:
        next_module = course.next_module(learning_module.id)
        return my_redirect(
            "/exam/quizzes/view_module/{0}/{1}".format(next_module.id, course_id)
        )
    ...
    if not first_unit:
        ...
        is_last_unit = course.is_last_unit(learning_module, current_learning_unit.id)
        if is_last_unit:
            next_module = course.next_module(learning_module.id)
            return my_redirect(
                "/exam/quizzes/view_module/{0}/{1}/".format(next_module.id, course.id)
            )
        ...
        if "change_prerequisite_completion" in request.POST:
            ...
            if unit_list:
                for unit in unit_list:
                    ...
                    learning_module.save()
                messages.success(request, "Changed prerequisite check successfully")
            else:
                messages.warning(request, "Please select atleast one module")
...
def preview_questionpaper(request, questionpaper_id):
    ...
    return my_render_to_response(request, "yaksh/preview_questionpaper.html", context)
...
def download_course(request, course_id):
    ...
    if (
        ...
        raise Http404("You are not allowed to download {0} course".format(course.name))
    if not course.has_lessons():
        raise Http404("{0} course does not have any lessons".format(course.name))
    current_dir = os.path.dirname(__file__)
    course_name = course.name.replace(" ", "_")

    # Static files required for styling in html template
    static_files = {
        "js": ["bootstrap.min.js", "jquery-3.3.1.min.js", "video.js"],
        "css": ["bootstrap.min.css", "video-js.css", "offline.css", "yakshcustom.css"],
        "images": ["lp.png"],
    }
    zip_file = course.create_zip(current_dir, static_files)
    zip_file.seek(0)
    response = HttpResponse(content_type="application/zip")
    response["Content-Disposition"] = "attachment; filename={0}.zip".format(course_name)
    response.write(zip_file.read())
    return response
...
def course_students(request, course_id):
    ...
    course = get_object_or_404(Course, pk=course_id)
    if not course.is_creator(user) and not course.is_teacher(user):
        raise Http404("You are not allowed to view {0}".format(course.name))
...
def view_notifications(request):
    user = request.user
    notifcations = Notification.objects.get_unread_receiver_notifications(user.id)
    if is_moderator(user):
        template = "manage.html"
    else:
        template = "user.html"
    ...
    return my_render_to_response(request, "yaksh/view_notifications.html", context)
...
def mark_notification(request, message_uid=None):
    user = request.user
    if message_uid:
        Notification.objects.mark_single_notification(user.id, message_uid, True)
    else:
        if request.method == "POST":
            msg_uuids = request.POST.getlist("uid")
            Notification.objects.mark_bulk_msg_notifications(user.id, msg_uuids, True)
...
def course_forum(request, course_id):
    ...
    if search_term:
        ...
        posts = course.post.get_queryset().filter(active=True).order_by("-modified_at")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_post.save()
            return redirect(
                "yaksh:post_comments", course_id=course.id, uuid=new_post.uid
            )
    else:
        form = PostForm()
    return render(
        request,
        "yaksh/course_forum.html",
        {
            "user": user,
            "course": course,
